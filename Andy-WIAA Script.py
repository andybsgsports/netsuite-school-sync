# Andy-WIAA Script.py
# ------------------------------------------------------------
# Improved version of Andy-School Script.py.
# Scrapes WIAA+IHSA, builds sport-tab workbook, diffs vs previous,
# emails changes, and pushes diffs to NetSuite.
#
# Uses shared scraper from netsuite_sync.py (no duplicate code).
# ------------------------------------------------------------
...
# Shared scraper + NetSuite sync
from netsuite_sync import scrape_wiaa_school_detail, decode_cf_email, sync_changes_to_netsuite
...
# ============= WIAA SCRAPING (uses shared scraper) =============
def extract_school_data(url, school):
    """Wrapper around shared scraper -- returns (admins, coaches) in Andy-format."""
    info, raw_admins, raw_coaches = scrape_wiaa_school_detail(url)
    ...
    for a in raw_admins:
        admins.append({
            "School": school.title(),
            "Role": canonical_admin_role_preserve_gender(a.get("role", "")),
            "First Name": a.get("first", ""),
            "Last Name": a.get("last", ""),
            "Email": a.get("email", ""),
            "State": state,
        })
    ...
    for c in raw_coaches:
        coaches.append({
            "School": school.title(),
            "Sport": c.get("role", ""),      # shared scraper puts sport in "role"
            "First Name": c.get("first", ""),
            "Last Name": c.get("last", ""),
            "Role": c.get("type", ""),        # shared scraper puts coach type in "type"
            "Email": c.get("email", ""),
            "State": state,
        })
...
def main():
    ...
    # WIAA scrape (using shared scraper)
    ...
    if SEND_EMAIL or SYNC_TO_NETSUITE:
        ...
        if SEND_EMAIL and send:
            try:
                ...
                print("[EMAIL] win32com not available (not on Windows?) -- skipping email.")
                shutil.copy2(FINAL_XLSX, PREV_XLSX)
        elif not send:
            print("[OK] No changes detected -- no email sent.")
    else:
        print("[INFO] Email and sync disabled.")
